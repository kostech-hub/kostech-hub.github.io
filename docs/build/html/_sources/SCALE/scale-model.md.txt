...
일반적으로 ![](images/model-icon.png){class="intxt-image"}**Model**의 권한 관리는 서로 다른 작업을 분리할 수 있는 역할 개념을 기반으로 합니다. 사용자는 자신이 권한이 있는 데이터(개별 데이터 또는 권한 그룹)에 대해서만 권한을 부여할 수 있습니다.
...
![alt text](images/parameter-replacement-of-strings.png)
...
- 선행되는 수 0이 최대 4자 길이인 Pool 버전: $ [% POOL_VERSION|format("0>4") %]
...
![](images/model-icon.png){class="intxt-image"}**Model**의 파라미터는 종종 프로세스를 제어하는 데 사용됩니다: 경로 지정, 솔버 설정 등입니다. 이러한 파라미터는 일반적으로 사용자/위치(user/location)에 따라 달라집니다. 따라서 ![](images/model-icon.png){class="intxt-image"}**Model**에 이러한 값을 저장하는 것은 최적이 아닙니다: 풀버전의 작성자가 설정한 설정이 다른 위치의 다른 사용자에게 전송될 수 있기 때문입니다.
...
2. 컨텍스트 메뉴에서 **Create run configuration**을 클릭합니다. 이때 만들어지는 실행 구성은 해당 그룹에 해당하는 속성 값과 상위 그룹의 속성 값을 자동으로 상속합니다.

![alt text](images/create-a-run-configuration.png)
    
#### Duplicate run configuration

실행 구성(run configuration)에 필요한 모든 속성을 설정하는 프로세스를 간소화하려면 기존 실행 구성의 복제본을 만든 다음 복제된 구성을 수정할 수 있습니다.

1. 복제하려는 실행 구성을 선택합니다.
2. 컨텍스트 메뉴에서 **Duplicate run configuration**을 클릭합니다.

:::{note}
둘 이상의 실행 구성(run configuration)을 선택하거나 Run view에서 상위 그룹의 컨텍스트 메뉴에서 **Duplicate run configruation**을 선택하여 여러개의 실행 구성을 복제 할 수 있습니다.
:::

![alt text](images/duplicate-a-run-configuration.png)

#### Export run configuration

이 기능을 사용하면 ![](images/model-icon.png){class="intxt-image"}**Model**에서 실행 구성(run configuration)을 JSON 형식 파일로 내보낼 수 있습니다. 내보낸 파일은 풀 버전(Pool version)의 컨텍스트 메뉴에 있는 파일에서 가져오기 옵션을 사용하여 ![](images/model-icon.png){class="intxt-image"}**Model**의 다른 설정으로 다시 가져올 수 있습니다. 이 기능은 테스트(TEST) 설정에서 프로덕션(production) 설정과 같이 서로 다른 서버 설정 간에 실행 구성을 복사할 때 유용합니다.

:::{important}
이 기능을 사용하려면 client를 --features=export_with_metadata로 시작해야 합니다. 이 기능이 활성화되면 실행 구성의 컨텍스트 메뉴에 Export run configuration...이 나타납니다.
:::

![alt text](images/export-run-configuration.png)

#### Remove a run configuration

선택한 실행 구성(run configuration)에 해당하는 풀 버전(Pool version)에서 선택한 실행 구성을 제거합니다.

![alt text](images/remove-run-configuration.png)



#### Edit a run configuration with external applications

이 옵션을 사용하면 외부 애플리케이션이 로컬 어셈블리(local assembly) 및 전처리기에서 모델 열기와 같은 작업을 위해 선택한 실행 구성(run configuration)의 컨텍스트(context)에 접근할 수 있습니다. 이 기능은 모델 편집을 위해 로드 케이스(load case)의 모든 구성 요소(all components)에 접근해야 할 때 특히 유용합니다. 이 기능을 사용하려면 외부 애플리케이션 스크립트를 명시적으로 구성해야 합니다.

![alt text](images/edit-run-configuration.png)

#### Copy and paste

실행 구성(run configuration)을 복사하여 다른 풀(Pool)에 붙여넣을 수 있습니다. 이 기능은 새 풀에서 처음부터 새 실행 구성을 만드는 대신 한 풀에서 실행 구성을 복제하여 다른 풀에 붙여넣은 다음 편집하려는 경우에 특히 유용합니다.

![alt text](images/copy-paste-run-configuration.png)

#### Add attachements, tags and markers

- Add Attachment : 파일을 첨부파일로 추가합니다.
- Tags : 미리 구성된 태그 또는 새 태그를 추가합니다.
- Markers : 마커를 추가하거나 제거합니다.

![alt text](images/add-attachments.png)

#### Lock run configuration

- A : 해당 작업은 ![alt text](images/lock-run-configuration-with-live.png){class="intxt-image"}로 표시된 라이브 버전에 속하는 풀(Pool)에서만 실행할 수 있습니다.
- B : 이 작업을 수행하면 잠금을 실행하는 사용자의 실행 구성(run configuration)이 잠깁니다. 이후에는 이 사용자만 실행 구성을 수정할 수 있습니다. 잠긴 실행 구성은 잠금 해제를 클릭하여 다시 잠금 해제할 수 있습니다.

![alt text](images/lock-run-configuration.png)

#### Show history

**Show history**를 클릭하면 별도의 탭에서 풀(Pool) 항목의 기록을 확인할 수 있습니다.

### Show changes

**Show changes** 확인란(A)를 활성화하면 변경 사항(Changes)이라는 레이블이(label)붙은 추가 열이 표시됩니다.(B) 

![alt text](images/show-changes-run-configuration.png)

### Edit a run configuration

실행 구성(run configuration)의 모든 설정은 Properties view에서 사용할 수 있습니다. 또한 그리드(grid)에서 실행 구성의 속성 값을 편집할 수도 있습니다.

#### Edit attributes in the grid

실행 구성(run configuration)을 선택하면 그리드(grid)에서 필터(filter) 행 아래의 첫 번째 행에 속성(attribute) 열 아래에 구성된 모든 속성 값이 표시됩니다. 선택한 실행 구성에 새 속성을 수정, 제거 또는 추가할 수 있습니다. 기존 속성을 제거하려면 해당 속성 값을 삭제하면 됩니다.

![alt text](images/edit-run-configuration-attributes.png)

1. 실행 구성(run configuration)을 선택합니다.
2. 표시되는 속성 값이 표시되는 행에서 원하는 항목을 더블 클릭하여 실행 구성(run configuration)의 속성(attribute) 값을 변경합니다.
3. 빈 필드(empty field)에 값을 추가하여 새 속성(attribute)을 추가합니다. 빈 속성(empty attribute) 값은 컴포넌트 매칭(component matching)에 고려되지 않습니다.

#### Edit attributes in the Properties view

실행 구성(run configuration)을 선택하면 속성 보기(Properties view)에서 실행 구성의 모든 속성과 함께 실행 구성과 관련된 추가 정보를 볼 수 있습니다. 기본 그리드(grid)에서 편집하는 것과 유사한 방식으로 속성 값을 편집할 수도 있습니다.

1. 속성 보기(Properties view)의 실행 구성(run configuration) 섹션 아래에는 실행 구성과 관련된 정보와 함께 모든 속성이 나열됩니다. 속성은 다음과 같이 더 분류됩니다.
2. Namescheme Attributes
3. Added Attributes - 사용된 subject가 컨텍스트(context)에 따라 추가로 추가 할 수 있는 속성입니다.
4. [Auxiliary Attributes](#auxiliary-attributes)
5. 속성(attribute) 값을 정의하기 위해 항목을 두 번 클릭 후 표시되는 드롭다운 메뉴에서 선택 할 수 있습니다.
6. - Select Attribute - 클릭 후 표시되는 드롭 다운 메뉴에서 속성(attribute)을 추가할 수 있습니다.
7. ![alt text](images/remove-attribute-icon.png){class="intxt-image" style="width: 16"}를 클릭 하여 속성(attribute)을 제거할 수 있습니다.

![alt text](images/edit-run-configuration-attributes-in-properties-view.png)

#### Edit properties of a run configuration

실행 구성(run configuration)의 속성 보기(Properties view)에서 다음과 같은 실행 구성의 속성을 변경할 수 있습니다.
...
![alt text](images/edit-properties-run-configuration.png)

### Assembly

- A : **Assemble locally** 실행 구성(run configuration)의 local assembly를 생성합니다.
- B : **Assembly remotely**은 실행 구성의 완성된 assembly를 구성된 HPC 시스템에 보냅니다.
- C : **Remote data check** 데이터 확인을 위한 실행을 구성된 HPC 시스템에 바로 보냅니다. 

![alt text](images/assembly-of-run-configuration.png)

### Multi run configuration (MRC)

다중 실행 구성(MRC)는 속성(attribute)값이 변경되는 실행 구성(run configuration)의 모음입니다. ![](images/model-icon.png){class="intxt-image"}**Model**은 풀 버전(Pool version)의 모든 파라미터 테이블(parameter table)을 검색하고 파라미터 테이블에서 일치하는 각 줄에 대해 하위 실행 구성을 생성합니다. 각 하위 실행 구성은 전체 실행 구성을 나타냅니다. 따라서 MRC는 각 변형(variation)을 수동으로 생성할 필요 없이 실행 구성의 변형을 생성하는데 특히 적합합니다. 응용 가능한 항목에는 견고성(robustness) 조사를 위한 배리어(barrier)의 위치 변화 또는 임팩터(impactor)에 대한 임팩트 영역(impact fields)의 생성이 있습니다. 

#### Create a multi run configuration

1. 단일 실행 구성(single run configuration)에서 다중 실행 구성(MRC)를 생성하기 위해 속성 보기(Properties view)에서 Multi run configuration 타입으로 변경합니다. 타입이 다중 실행 구성으로 설정되면 다중 속성 및 파라미터 테이블이라는 추가 속성이 표시됩니다.

2. 다중 속성(Multi Attribute)를 사용하면 각각 생성되는 실행 구성에 변경되어 적용되어야 하는 속성 값을 할당 할 수 있습니다.

3. 파라미터 테이블(parameter table)은 하위 실행 구성(child run configuration)을 생성할 풀 버전에서 파라미터 테이블을 선택할 수 있도록 합니다. 이 옵션을 명시적으로 정의하지 않으면 하위 실행 구성을 만들 때 풀 버전(Pool version)의 모든 파라미터 테이블이 고려됩니다.

![alt text](images/create-multi-run-configuration.png)

___

![alt text](images/parameter-table-with-value-assignments.png)

- A : 다중 실행 구성에서 각 실행 구성에 변경되는 값으로 적용될 항목의 속성(attribute)입니다.
- B : 다중 실행 구성에서 선택한 속성(attribute)에서 일치하는 항목들을 함께 표시합니다.
- C : 다중 실행 구성에서 선택한 속성(attribute)에서 일치하지 않는 항목들을 구분하여 표시합니다.

___

![alt text](images/automatically-generated-child-run-configuration.png)

- A : 정의된 다중 실행 구성(MRC)입니다.
- B : ***Multi Attribute***와 ***Parameter Tables***에서 선택된 항목들에 기반하여 자동으로 생성된 하위 실행 구성(child run configuration)입니다.
