...
:::{attention}
여기서는 local 환경(On client)에서 R13.1.0의 smp single 솔버를 사용하며 LSOPT 7.0 버전으로 하였습니다. 필요에 따라 LS-Dyna 솔버를 변경하시고 mpp 솔버를 사용하는 경우 MPI Path에 MPI 경로를 설정하셔야 합니다. MPI Path는 mpirun, mpiexec 파일이 존재하는 폴더 경로입니다. LSOPT는 7.0에서 테스트 되었기 때문에 상위버전에서 정상적으로 진행되지 않을 가능성이 있습니다. 서버 장비(On server)에서 LSOPT 최적화 시뮬레이션을 진행하기 위해서는 사용 환경에 맞추어 테스트가 요구되기 때문에 사용을 원하시면 KOSTECH으로 문의주시기 바랍니다.
:::
